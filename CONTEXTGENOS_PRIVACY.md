# ContextGenOS Privacy & Security Architecture
...
Before explaining what ContextGenOS does, it's worth being precise about what currently exists and exactly where each solution fails, because the failures are not minor inconveniences. They are structural, and they are by design.
...
## The ContextGenOS Model: Architecture as Privacy

ContextGenOS does not treat privacy as a settings menu or a compliance checkbox. Privacy in ContextGenOS is an architectural property, meaning it is enforced by how the system is built, not by how the system is configured.
...
ContextGenOS runs entirely on your machine. There is no ContextGenOS server. There is no ContextGenOS cloud. There is no company database that aggregates user context. The software you install is the entirety of the system, and it operates exclusively on your hardware.

What this means in practice:

**No network requests at runtime.** The ContextGenOS daemon, once installed, does not make outbound network connections during normal operation. It reads from your local data sources, writes to your local store, and serves context to local applications. You can verify this with any network monitoring tool. We encourage you to do so.

**No telemetry.** ContextGenOS collects no usage data, no error reports, no "anonymous analytics." We know that most software says this. The difference here is that you can verify it — the codebase is open, the binary is reproducible, and the network behavior is auditable. Trust should be verifiable, not demanded.

**No authentication to a remote service.** You do not create a ContextGenOS account. You do not log in. There is no server that could be breached and expose your credentials or your data, because there is no server.

**Your data has one location.** Your context store lives at a path on your filesystem that you can inspect, back up, encrypt with your own tools, or delete entirely. At any moment, you can `rm -rf ~/.contextgenos` and every piece of data ContextGenOS holds is gone. No deletion requests. No "we'll delete it within 30 days." Gone, immediately, because it was always yours.

### Encryption Architecture

ContextGenOS implements encryption in a fundamentally different way than cloud services.

**You hold the keys.** The local context store is encrypted using keys derived from your device's secure enclave (on supported hardware: Apple Silicon, TPM on Windows/Linux) or a user-provided passphrase. ContextGenOS never transmits your keys. ContextGenOS never generates keys on a remote server. The mathematical relationship between your keys and your data means that no one who does not possess your keys — including the authors of ContextGenOS — can read your context store.

**Key derivation.** On first initialization, ContextGenOS derives an encryption key using your device's secure hardware where available. On Apple Silicon, this uses the Secure Enclave. On Linux with TPM, this uses the TPM chip. On systems without secure hardware, a passphrase-derived key (Argon2id) is used. The derived key never leaves your device.

**Encrypted at rest, controlled by you.** The DuckDB structured store and the Qdrant vector index are both encrypted at rest. The encryption is performed by ContextGenOS before data is written to disk. Unlike cloud "encrypted at rest," there is no server-side decryption layer between your data and any third party.

**Optional additional encryption.** Users who require higher assurance can configure ContextGenOS to encrypt the entire data directory with a separate passphrase required at daemon startup. This means the context store is unusable even if someone gains physical access to your machine while it is powered off.

### The Privacy Rules Engine

Not all context is equally sensitive, and not all context is appropriate to share with every AI tool. ContextGenOS includes a privacy rules engine that gives you fine-grained control over what context is captured and what context is injected.

**Capture rules:** Control what ContextGenOS collects from each source:
```yaml
collectors:
  browser:
    exclude_domains:
...
**Context tagging.** Every piece of context captured by ContextGenOS is automatically tagged with a sensitivity classification (work, personal, health, financial, relationship) based on its source and content. These tags drive injection decisions. You can review and override tags at any time through the inspection UI.

### The Inspection Guarantee

ContextGenOS maintains a complete, human-readable log of every context injection event. Before any piece of your context is provided to any AI tool, a log entry is created containing:
...
This log is accessible through the ContextGenOS UI and queryable via CLI:

```bash
# What did ContextGenOS inject into my Claude conversation this morning?
contextgenos log inspect --since "09:00" --model claude

# Has anything from my health data ever been injected?
contextgenos log search --sensitivity health

# What does ContextGenOS know that matched my last query?
contextgenos log last --verbose
```
...
## Threat Model: What ContextGenOS Protects Against

Being precise about what ContextGenOS protects against, and what it does not, is more honest and more useful than vague privacy claims.

### Protected Against

**Corporate data harvesting.** ContextGenOS has no server to harvest to. There is no mechanism by which Anthropic, any company, or ContextGenOS contributors can access your context data.
...
**Data broker resale.** Cloud AI memory services operate in a legal environment where user data can be sold, licensed, or transferred in acquisition scenarios. ContextGenOS data is on your machine and is not subject to any company's data practices.

**Breach of a cloud provider.** If ContextGenOS had a server and that server was breached, there would be no user context data to expose. The attack surface that cloud providers represent — a single database containing the accumulated context of millions of users — does not exist with ContextGenOS.

**Legal demands served on a third party.** A subpoena, National Security Letter, or court order served on a cloud provider can compel disclosure of your data without your knowledge. With ContextGenOS, there is no third party holding your data. A legal demand would have to be served on you directly, which means you are notified and have the ability to respond.

**Passive context leakage across models.** With cloud memory, context accumulated from your interactions with one model may inform the training of another, or be accessible across product lines within a company's ecosystem. With ContextGenOS, the same context store serves multiple models, but the data never leaves your machine: you are choosing which models receive context from a local system, not allowing a corporation to connect your data across their products without your awareness.

**Account compromise.** There is no ContextGenOS account to compromise. Your context cannot be accessed by someone who gains access to a ContextGenOS account because no such account exists.

### Not Protected Against

**Malware on your machine.** If an attacker has code execution on your device, they can potentially access your context store. ContextGenOS uses encryption to raise the cost of this attack but cannot fully prevent it if your device is compromised. Device security is a prerequisite for ContextGenOS security.

**Physical device access.** If someone has physical access to your powered-on device and you have not locked your session, they can access the running ContextGenOS daemon. Standard device security practices apply.

**The AI models themselves.** ContextGenOS injects your context into prompts sent to AI models — including cloud models. If you choose to inject context into a cloud model like GPT-4, that context leaves your machine as part of the prompt. ContextGenOS's privacy rules engine allows you to restrict what is injected into cloud versus local models, but the act of injection itself sends data to the model provider. ContextGenOS is transparent about this. The injection log shows exactly what was sent to which model.

**Legal demands served directly on you.** ContextGenOS protects your data from third-party legal demands. It does not protect you from legal demands directed at you personally.

---

## Comparison Table

| Property | ContextGenOS | Cloud Memory (Mem.ai etc.) | App Memory (ChatGPT etc.) | Notes + AI Plugins |
|---|---|---|---|---|
...
ContextGenOS is structurally compliant with the strongest possible interpretation of these regulations — not because we have designed for compliance, but because the architecture that best serves users also happens to be the architecture that regulators are moving toward requiring. No personal data leaves the user's device. No processing occurs on company infrastructure. No retention policies are needed because no retention occurs outside the user's control.

As regulation tightens, cloud AI memory services will need to build toward what ContextGenOS already is. ContextGenOS starts there.

---

## Summary

ContextGenOS is not more private than cloud alternatives because it has better privacy settings or stronger legal policies. It is more private because the attack surface that enables privacy violations: a server holding your data, a company holding your encryption keys, a training pipeline that can consume your context, does not exist.
